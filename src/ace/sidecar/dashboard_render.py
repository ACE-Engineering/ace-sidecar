...
def _prometheus_section(d: Dict[str, Any]) -> str:
    ...
    rows = [
        ("ace_sessions_total", "Counter", "agent", "Total observed developer session transcripts."),
        ("ace_turns_total", "Counter", "agent", "Total AI agent API turns / requests processed."),
        ("ace_tokens_input_fresh_total", "Counter", "—", "Fresh (uncached) prompt input tokens."),
        ("ace_tokens_cache_read_total", "Counter", "—", "Input tokens served directly from Anthropic prompt cache."),
        ...
        ("ace_cache_read_share", "Gauge", "—", "Ratio of prompt input tokens served from cache."),
    ...
    return (
        ...
        "<span class='pill on' style='background:#0F231A;color:var(--mint);border-color:#1d3b2e;'>✓ MACHINE-READABLE</span>"
        "<span class='pill on' style='background:#0d1c33;color:var(--blue);border-color:#1e355b;'>⚡ PROMETHEUS TEXT FORMAT</span>"
        "</div></div>"
        "<p>The ACE sidecar exposes standard Prometheus text exposition format metrics at <code>http://127.0.0.1:8787/metrics</code>. "
        "This endpoint is designed for time-series scraping into Prometheus, Grafana Alloy, OpenTelemetry Collector, VictoriaMetrics, Datadog, Snowflake, or ClickHouse.</p>"
        ...
        "<div class='ph'><span>Prometheus Metrics Schema Reference</span>"
        "<span class='live'><i></i>16 METRICS EMITTED</span></div>"
        "<div class='pb'><table>"
        ...
        "<div class='pan'><div class='ph'><span>Prometheus Scrape Config (prometheus.yml)</span></div>"
        "<div class='pb'><div class='calcbox' style='margin:0;'><pre style='color:#a3e635;margin:0;font-family:var(--mono);font-size:12px;'>scrape_configs:\n  - job_name: 'ace_sidecar'\n    scrape_interval: 15s\n    static_configs:\n      - targets: ['127.0.0.1:8787']</pre></div></div></div>"
        "<div class='pan'><div class='ph'><span>OpenTelemetry Collector Config</span></div>"
        "<div class='pb'><div class='calcbox' style='margin:0;'><pre style='color:var(--blue);margin:0;font-family:var(--mono);font-size:12px;'>receivers:\n  prometheus:\n    config:\n      scrape_configs:\n        - job_name: 'ace_sidecar'\n          static_configs:\n            - targets: ['127.0.0.1:8787']</pre></div></div></div>"
        "</div>"
        "<div class='pan'><div class='ph'><span>Live Stream Inspector</span>"
        ...
        "<div class='pb'><div id='sectionMetricsPreviewBox' class='metrics-box' style='margin:0;max-height:220px;'>Click 'Fetch Live Payload' to view live metrics stream...</div></div></div>"
    )
...
<div id='metricsModal' class='modal-overlay' onclick='if(event.target===this)this.style.display="none"'>
  <div class='modal-content'>
    ...
    <div style='margin-bottom:16px;'>
      ...
      <span class='pill on' style='background:#0d1c33;color:var(--blue);border-color:#1e355b;margin-left:6px;'>MACHINE READABLE</span>
    </div>
    ...
    <div style='display:flex;gap:10px;margin-bottom:16px;align-items:center;'>
      <input type='text' readonly id='metricsUrlInput' value='http://127.0.0.1:8787/metrics' style='flex:1;background:#0b0c0d;border:1px solid #282e30;color:var(--mint);padding:8px 12px;border-radius:4px;font-family:var(--mono);font-size:13px;' />
    ...
    <div style='font-weight:600;font-size:13.5px;margin-bottom:8px;'>Prometheus Scrape Config Example</div>
    <div class='metrics-box' style='max-height:none;color:#a3e635;margin-bottom:16px;'>scrape_configs:
  - job_name: 'ace_sidecar'
    scrape_interval: 15s
    static_configs:
      - targets: ['127.0.0.1:8787']</div>
    <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;'>
      <span style='font-weight:600;font-size:13.5px;'>Live Payload Preview</span>
      <button class='btn' style='padding:4px 10px;font-size:12px;cursor:pointer;' onclick='loadLiveMetrics()'>Fetch Live Stream</button>
    </div>
    <div id='metricsPreviewBox' class='metrics-box'>Click 'Fetch Live Stream' to load current payload...</div>
  </div>
</div>
"""
...
async function loadLiveMetrics() {
  ...
  box.innerText = 'Loading metrics stream...';
  try {
